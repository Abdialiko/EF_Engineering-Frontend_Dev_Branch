      ...
      <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#05225c" strokeWidth="1" opacity="0.5"/>
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#grid)" />
      </svg>
      <div className="absolute inset-0 bg-gradient-to-br from-transparent via-[#05225c]/5 to-transparent animate-gradient-shift"></div>
    </div>
  );
};
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-10">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="absolute h-px bg-gradient-to-r from-transparent via-[#05225c] to-transparent animate-slide-in-right"
          style={{
            top: `${20 + i * 20}%`,
            left: '-100%',
            width: '200%',
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-5">
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="absolute animate-geometric-move will-change-transform"
          style={{
            width: `${40 + i * 10}px`,
            height: `${40 + i * 10}px`,
            left: `${10 + i * 15}%`,
            top: `${10 + i * 12}%`,
            border: `2px solid ${i % 2 === 0 ? '#05225c' : '#e34b11'}`,
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {particles.map((particle, i) => (
        <div
          key={i}
          className="absolute w-1 h-1 bg-[#05225c] rounded-full animate-float will-change-transform"
          style={{
            left: `${particle.left}%`,
            top: `${particle.top}%`,
      ...
      <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="blueprint" width="100" height="100" patternUnits="userSpaceOnUse">
            <circle cx="50" cy="50" r="2" fill="#05225c" opacity="0.6"/>
            <line x1="0" y1="50" x2="100" y2="50" stroke="#05225c" strokeWidth="0.5" opacity="0.4"/>
            <line x1="50" y1="0" x2="50" y2="100" stroke="#05225c" strokeWidth="0.5" opacity="0.4"/>
          </pattern>
        </defs>
  ...
  return (
    ...
    >
      <path
        ...
        fill="currentColor"
      />
      <rect x="9" y="14" width="2" height="4" fill="#e34b11" />
      <rect x="13" y="14" width="2" height="4" fill="#e34b11" />
    </svg>
  );
};
